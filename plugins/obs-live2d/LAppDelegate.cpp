...
const LPCWSTR ClassName = L"Cubism DirectX11 Sample";
...
bool LAppDelegate::Initialize()
{
	...
	_windowHandle = CreateWindow(ClassName, ClassName, WS_OVERLAPPEDWINDOW,
				     CW_USEDEFAULT, CW_USEDEFAULT, rect.right,
				     rect.bottom, NULL, NULL,
				     _windowClass.hInstance, NULL);
	if (_windowHandle == NULL) {
		blog(LOG_ERROR, "can't create Window");
		return false;
	}
	...
	HRESULT result = D3D11CreateDeviceAndSwapChain(
		...
		0,    // ↑の要素数
		D3D11_SDK_VERSION, &_presentParameters, &_swapChain, &_device,
		&level, &_deviceContext);
	if (FAILED(result)) {
		blog(LOG_ERROR, "fail Direct3D Create Device 0x%x", result);
		return false;
	}

	// RenderTargetView/DepthStencilViewの作成
	CreateRenderTarget(static_cast<UINT>(LAppDefine::RenderTargetWidth),
			   static_cast<UINT>(LAppDefine::RenderTargetHeight));
	...
	if (FAILED(result)) {
		blog(LOG_ERROR, "can't create Depth 0x%x", result);
		return false;
	}
	...
	if (FAILED(result)) {
		blog(LOG_ERROR, "can't create Rasterizer 0x%x", result);
		return false;
	}
	...
	if (FAILED(result)) {
		blog(LOG_ERROR, "can't create Sampler 0x%x", result);
		return false;
	}
...
{
	if (!_swapChain || !_device) { // SwapChain、Deviceが無い場合は無理
		return false;
	}
	...
	hr = _swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D),
				   (LPVOID *)&backBuffer);
	if (FAILED(hr)) {
		blog(LOG_ERROR, "can't SwapChain GetBuffer 0x%x", hr);
		return false;
	}
	hr = _device->CreateRenderTargetView(backBuffer, NULL,
					     &_renderTargetView);
	// Getした分はRelease
	backBuffer->Release();
	if (FAILED(hr)) {
		blog(LOG_ERROR, "can't create RenderTargetView 0x%x", hr);
		return false;
	}
	...
	if (FAILED(hr)) {
		blog(LOG_ERROR, "can't create DepthTarget 0x%x", hr);
		return false;
	}
	// DepthView
	...
	hr = _device->CreateDepthStencilView(_depthTexture, &depthViewDesc,
					     &_depthStencilView);
	if (FAILED(hr)) {
		blog(LOG_ERROR, "can't create DepthTargetView 0x%x", hr);
		return false;
	}
...
void LAppDelegate::InitializeCubism()
{
	...
	Live2D::Cubism::Framework::Rendering::CubismRenderer_D3D11::
		InitializeConstantSettings(BackBufferNum, _device);
...
bool LAppDelegate::CreateShader()
{
	...
	do {
		...
		if (FAILED(hr)) {
			blog(LOG_ERROR, "can't compile Vertex Shader");
			break;
		}
		hr = _device->CreateVertexShader(vertexBlob->GetBufferPointer(),
						 vertexBlob->GetBufferSize(),
						 NULL, &_vertexShader);
		if (FAILED(hr)) {
			blog(LOG_ERROR, "can't create Vertex Shader");
			break;
		}
		...
		if (FAILED(hr)) {
			blog(LOG_ERROR, "can't compile Pixel Shader");
			break;
		}

		hr = _device->CreatePixelShader(pixelBlob->GetBufferPointer(),
						pixelBlob->GetBufferSize(),
						NULL, &_pixelShader);
		if (FAILED(hr)) {
			blog(LOG_ERROR, "can't create Pixel Shader");
			break;
		}
		...
		hr = _device->CreateInputLayout(elems, ARRAYSIZE(elems),
		...
		if (FAILED(hr)) {
			blog(LOG_ERROR,
			     "can't create VertexDeclaration failed");
			break;
		}
	...
	blendDesc.IndependentBlendEnable =
		FALSE; // falseの場合はRenderTarget[0]しか使用しなくなる
...
{
	if (_device == NULL || _vertexFormat == NULL || _vertexShader == NULL ||
	    _pixelShader == NULL) {
		return;
	}
...
void LAppDelegate::StartFrame()
{
	...
	_deviceContext->ClearDepthStencilView(
		_depthStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL,
		1.0f, 0);
...
void LAppDelegate::EndFrame()
{
	...
	if (hr == DXGI_ERROR_DEVICE_REMOVED ||
	    hr == DXGI_ERROR_DEVICE_RESET) { // デバイスロストチェック
	...
	if (_deviceStep == DeviceStep_Lost) { // ロストした
		blog(LOG_ERROR, "device losted");
		// アプリケーション終了
		AppEnd();
	}
...
void LAppDelegate::ResizeDevice()
{
	...
	// デバイス設定
	if (nowWidth == 0 ||
	    nowHeight == 0) // サイズが0の際は最小化されていると思われる
	{
		// NOP サイズが戻ったら再チャレンジ
	} else {
		// ターゲットがあるなら削除
		...
		if (_swapChain) {
			...
			if (hr == DXGI_ERROR_DEVICE_REMOVED ||
			    hr == DXGI_ERROR_DEVICE_RESET) { // デバイスがロスト
				_deviceStep = DeviceStep_Lost;
			} else {
...
LRESULT WINAPI LAppDelegate::MsgProc(HWND wnd, UINT msg, WPARAM wParam,
				     LPARAM lParam)
{
	switch (msg) {
	case WM_DESTROY: // 終了
	...
	case WM_SIZE: // ウィンドウサイズ変更
		if (s_instance != NULL && s_instance->_view != NULL &&
		    s_instance->_device &&
		    s_instance->_deviceStep !=
			    DeviceStep_Lost) { // _device作成前にCreateWindowをやった時もここに来るので、_deviceのNullチェックは必須
			// Resize指示を出す
