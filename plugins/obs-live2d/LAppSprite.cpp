	  ...
	  _constantBuffer(NULL)
{
	...
	if (!_vertexBuffer) {
		...
		bufferDesc.ByteWidth = sizeof(LAppSprite::SpriteVertex) *
				       VERTEX_NUM; // 総長 構造体サイズ*個数
	...
	if (!_indexBuffer) {
		...
		bufferDesc.ByteWidth =
			sizeof(WORD) * INDEX_NUM; // 総長 構造体サイズ*個数
	...
	if (!_constantBuffer) {
		...
		bufferDesc.ByteWidth = sizeof(
			CubismConstantBufferD3D11); // 総長 構造体サイズ*個数
		bufferDesc.Usage =
			D3D11_USAGE_DEFAULT; // 定数バッファに関しては「Map用にDynamic」にしなくともよい
...
void LAppSprite::Render(int width, int height) const
{
	...
	vtx[0].x = (_rect.left - width * 0.5f) / (width * 0.5f);
	vtx[0].y = (_rect.down - height * 0.5f) / (height * 0.5f);
	vtx[1].x = (_rect.right - width * 0.5f) / (width * 0.5f);
	vtx[1].y = (_rect.down - height * 0.5f) / (height * 0.5f);
	vtx[2].x = (_rect.left - width * 0.5f) / (width * 0.5f);
	vtx[2].y = (_rect.up - height * 0.5f) / (height * 0.5f);
	vtx[3].x = (_rect.right - width * 0.5f) / (width * 0.5f);
	vtx[3].y = (_rect.up - height * 0.5f) / (height * 0.5f);

	// 頂点書き込み
	if (_vertexBuffer) {
		D3D11_MAPPED_SUBRESOURCE subRes;
		if (SUCCEEDED(renderContext->Map(_vertexBuffer, 0,
						 D3D11_MAP_WRITE_DISCARD, 0,
						 &subRes))) {
			memcpy(subRes.pData, vtx,
			       sizeof(SpriteVertex) * VERTEX_NUM);
	...
	if (_constantBuffer) {
		...
		renderContext->UpdateSubresource(_constantBuffer, 0, NULL, &cb,
						 0, 0);
...

	{
		...
		renderContext->IASetVertexBuffers(0, 1, &_vertexBuffer,
						  &strides, &offsets);
		renderContext->IASetIndexBuffer(_indexBuffer,
		...
		if (LAppDelegate::GetInstance()->GetTextureManager()->GetTexture(
			    _textureId, textureView)) {
...
void LAppSprite::RenderImmidiate(int width, int height,
				 ID3D11ShaderResourceView *resourceView) const
{
...

	{
		...
		// テクスチャセット
		{
			renderContext->PSSetShaderResources(0, 1,
							    &resourceView);
		}
...
bool LAppSprite::IsHit(float pointX, float pointY) const
{
	...
	LAppPal::CoordinateWindowToFullScreen(static_cast<float>(clientWidth),
					      static_cast<float>(clientHeight),
					      pointX, pointY, coordX, coordY);

	if (clientWidth == 0 || clientHeight == 0) { // この際はヒットしない
		return false;
	}
	...
	return (coordX >= _rect.left && coordX <= _rect.right &&
		coordY <= _rect.up && coordY >= _rect.down);
}
