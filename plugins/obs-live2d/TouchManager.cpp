...
{
	float distance =
		CalculateDistance(deviceX1, deviceY1, deviceX2, deviceY2);
	...
	if (_lastTouchDistance > 0.0f) {
		...
		_deltaX = CalculateMovingAmount(deviceX1 - _lastX1,
						deviceX2 - _lastX2);
		_deltaY = CalculateMovingAmount(deviceY1 - _lastY1,
						deviceY2 - _lastY2);
	} else {
		_scale = 1.0f;
		_deltaX = 0.0f;
...
float TouchManager::CalculateMovingAmount(float v1, float v2)
{
	...
	return sign * ((absoluteValue1 < absoluteValue2) ? absoluteValue1
							 : absoluteValue2);
}
